...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  await cargarCotizacionestaxExempt();
  configurarEventos();
...
function filtrarCotizaciones(termino) {
  // Primero filtramos solo las cotizaciones Tax Exempt
  let cotizacionesFiltradas = cotizacionesGuardadas.filter(c => esTaxExempt(c));
...
async function cargarCotizacionestaxExempt() {
  ...
  try {
    ...
    const cotizacionesTaxExempt = cotizacionesGuardadas.filter(c => esTaxExempt(c));
    actualizarListaCotizaciones(cotizacionesTaxExempt);
    document.getElementById('badgeTaxExempt').textContent = cotizacionesTaxExempt.length;
  } catch (error) {
...
function actualizarListaCotizaciones(cotizaciones) {
  ...
  if (cotizaciones.length === 0) {
    container.innerHTML = '<p class="texto-vacio">No hay cotizaciones con Tax Exempt.</p>';
    return;
  }
  ...
  cotizaciones.forEach(cotizacion => {
    ...
    const status = cotizacion.status || 'en_proceso';
