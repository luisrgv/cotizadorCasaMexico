...
document.addEventListener('DOMContentLoaded', async () => {
  await verificarSesion();
    await cargarcotizacionesCancelado();
  configurarEventos();
...
async function cargarcotizacionesCancelado () {
  ...
  try {
    ...
    const cargarcotizacionesCancelado = cotizacionesGuardadas.filter(c => c.status === 'cancelado');
    actualizarListaCotizaciones(cargarcotizacionesCancelado);
    document.getElementById('badgeCanceladas').textContent = cargarcotizacionesCancelado.length;
  } catch (error) {
...
function actualizarListaCotizaciones(cotizaciones) {
  const container = document.getElementById('cotizacionesCancelado');
  container.innerHTML = '';
...
function configurarEventos() {
  ...
  document.getElementById('cotizacionesCancelado').addEventListener('click', (event) => {
  ...
  if (e.target.closest('.eliminar-cotizacion')) {
    ...
    if (confirm('¿Estás seguro que deseas eliminar esta cotización? Esta acción no se puede deshacer.')) {
      try {
        ...
        await cargarcotizacionesCancelado(); // refrescar lista
      } catch (error) {
